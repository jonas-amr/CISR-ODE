...
public:
	...
	{
		u.theta_acc_ref()=arma::vec({
			0.0,
			+std::min(20.0/3.0,2*t),
			-std::min(20.0/3.0,2*t)
		});
	...
	static arma::vec::fixed<3> derivative_range3(const arma::vec::fixed<3> vec_now,const arma::vec::fixed<3> vec_last_observed,const double t,const double last_observed_t)
	{
		...
		if(dt==0.0)
			...
			D_vec=(vec_now-vec_last_observed)/dt;
		return D_vec;
	}

	static arma::vec::fixed<3> limit_D_states3(const arma::vec::fixed<3> x,const arma::vec::fixed<3> x_dot,double limit_x)
	{
		arma::vec::fixed<3> DX_limited=x_dot;
		for(uint k=0;k<2;k++) // why k<3 gives a bad result?
			if(std::abs(x(k))>=limit_x && x(k)*x_dot(k)>0)
				DX_limited(k)=0.0; // integral limit
		return DX_limited;
	}
